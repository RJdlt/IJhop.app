-- 0020: Pontdeals in het dashboard. Volledige vervanging, zoals eerder.
-- Draai 0019 eerst (die maakt de tabellen), daarna dit bestand.
--
-- Wat erbij komt, onder de sleutel `deals`:
--
-- 1) Per deal de trechter kaart gezien -> pak je deal -> code getoond ->
--    ingewisseld. De eerste drie stappen komen uit `analytics_events` en
--    tellen unieke gebruikers; de laatste komt uit `deal_codes`, want dat is
--    de enige plek waar inwisselen echt gebeurt. Een event kan liegen, een
--    rij met `redeemed_at` niet.
--
-- 2) Dezelfde trechter per steiger, zodat zichtbaar wordt of een deal het
--    slecht doet of gewoon bij de verkeerde pont hangt.
--
-- 3) `deal_return`: heeft een deal terugkeer opgeleverd? Per week vergelijken
--    we twee groepen actieve gebruikers: wie in die week een code pakte, en
--    wie niet. Van allebei tellen we hoeveel er de week erna terugkwamen.
--    Dat is geen bewijs van oorzaak (wie een deal pakt was toch al
--    enthousiaster), maar het verschil is het eerste dat je wilt zien.
--    Weken die nog niet voorbij zijn tellen niet mee.
--
-- De uitsluiting van test-sessies en eigen admin-activiteit werkt hier
-- hetzelfde als in de rest van de functie: alles hangt aan `ev`.
...
begin
  ...
  deal_rows as ( -- alle deals die in het venster liepen, met partner en steiger
    select d.id::text as deal_id, d.offer, d.stop_id, p.name as partner,
           ...
           (select count(*)::int from public.deal_codes c where c.deal_id = d.id) as codes,
           (select count(*)::int from public.deal_codes c
             where c.deal_id = d.id and c.redeemed_at is not null) as redeemed
    from public.deals d
