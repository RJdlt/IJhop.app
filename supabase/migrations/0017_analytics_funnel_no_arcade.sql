-- 0017: funnel zonder arcade. Draai dit ná 0016 in de Supabase SQL-editor.
--
-- Dit bestand vervangt de hele functie, dus wie 0016 nog niet gedraaid heeft
-- kan direct dit bestand plakken; alles uit 0015 en 0016 (venster "Alles",
-- weekreeks, nieuw vs terugkerend, eerlijke sessieduur) zit hierin.
--
-- Wat er verandert ten opzichte van 0016:
--
-- 1) De spellen zijn uit de app verdwenen (6% bereikte de arcade, 4% maakte
--    een spel af), dus de oude funnel-stappen `arcade`, `started` en
--    `finished` meten niets meer. De funnel volgt nu de weg die iedereen
--    wél loopt:
--
--      App geopend  -> elke sessie met minstens één event
--      Klok gezien  -> de sessie bleef minstens één heartbeat (30 seconden)
--                      zichtbaar, of deed iets in de app (pont gekozen,
--                      storing gemeld of weggeklikt, ontmoeting gestart).
--                      Alleen `session_start` betekent: geopend en meteen weg.
--      Melding aan  -> `push_subscribe` gelukt. Deze stap staat er nu al in,
--                      maar zal pas oplopen als de meldingen echt live gaan.
--
--    De stappen zijn genest: wie een melding aanzette heeft de klok gezien.
--
-- 2) `tabs` en `characters` verdwijnen uit het antwoord. `tab_view` bestond
--    alleen om te meten of iemand naar het spellen-tabblad ging en er is nu
--    maar één scherm; `character_select` hoorde bij de arcade-winkel. Oude
--    rijen blijven gewoon in de tabel staan, ze worden alleen niet meer
--    samengevat.
